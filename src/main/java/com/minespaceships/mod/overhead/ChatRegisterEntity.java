...
public class ChatRegisterEntity extends TileEntity implements IMoveable {
	...
	public ChatRegisterEntity() {
		remoteWorld = (WorldServer) MinecraftServer.getServer()
				.getEntityWorld();
	}
	...
	public void writeToNBT(NBTTagCompound par1) {
		...
		if (ship != null) {
			par1.setIntArray(recoverSpaceshipMeasures,
					ship.getOriginMeasurementArray());
		}
	}
	...
	public void Activate(EntityPlayer player) {
		// check if the player is our local player, so one cannot open a console
		// for another player
		// on the server
		if (player.equals(Minecraft.getMinecraft().thePlayer)) {
			// initialise the terminal
			// terminal = new CustomGuiChat(player,
			// (ChatRegisterEntity)remoteWorld.getTileEntity(pos));
	...
	public void onCommand(String command, EntityPlayer player) {
		...
		if (command.equals("hello")) {
			// send something to the player to see if we get a feedback from our
			// command.
			...
			SpaceshipCommands.init("init -4;-4;-4 to 4;4;4", remoteWorld, this,
					player, ship);
			...
			if (command.equals("left")) {
				...
				player.addChatComponentMessage(new ChatComponentText(
						"Invalid direction! Only left, right or around!"));
			}
		} else if (command.equals("status")) {
			SpaceshipCommands.status(remoteWorld, this, player, ship);
		} else if (command.startsWith("phaser")) {

			// TODO: Move this to an appropriate class
			command = command.substring("phaser".length())
					.replaceAll("\\s", "");
			...
			if (moffset.matches()) {
				Vec3 direction = new Vec3(Double.valueOf(moffset.group(1)),
						Double.valueOf(moffset.group(2)),
						Double.valueOf(moffset.group(3)));

				player.addChatComponentMessage(new ChatComponentText("Test-shooting Phaser with 10 strength and 100 max range."));
				PhaserUtils.shoot(pos, direction, 10, 100, remoteWorld);
			} else {
				player.addChatComponentMessage(new ChatComponentText("Input error. Usage: phaser #;#;#"));
			}
